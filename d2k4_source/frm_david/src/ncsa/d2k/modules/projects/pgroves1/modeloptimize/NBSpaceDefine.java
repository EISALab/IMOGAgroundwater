...
public class NBSpaceDefine extends DataPrepModule
	{
	...
	//		firstRun=false;
		...
		if(debug)
			System.out.println("NBSpaceDefine: isReady "+b);
		return b;
	}
	...
	*/
	public void doit() throws Exception{
		...
		if(debug)
			System.out.println("NBSpaceDefine: pushing Space");
	...
	*/
	public static Range[] getRanges(){
		IntRange[] ranges=new IntRange[2];
		
		ranges[0]=new IntRange("BinMethod",1, 0);
		ranges[1]=new IntRange("Count",50,2);
		return ranges;
	}
	/**
	...
	*/
	...
	public String getModuleInfo(){
		return "<html>  <head>      </head>  <body>    Makes a single objective search space where error minimization is the objective and the biases of an Binning scheme for a NaiveBayesModelGenerator are the search space parameters.<br><b>Properties</b>    <ul>      <li>        <b>ConvergenceTarget</b>: the search space object requires this. If we are actually trying to optimize a model's parameters, this is the threshold error value to let the optimizer stop at when reached </li>    </ul>  </body></html>";
	}

   	public String getModuleName() {
		return "Binning Search Space";
		}
