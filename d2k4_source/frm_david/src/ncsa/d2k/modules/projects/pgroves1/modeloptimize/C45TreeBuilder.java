package ncsa.d2k.modules.projects.pgroves.modeloptimize;//core.prediction.decisiontree;
...
*/
public class C45TreeBuilder extends /*ReentrantComputeModule*/OrderedReentrantModule {
	...
	*/
	...
	private void setMinimumRecordsPerLeaf(int num) {
		minimumRecordsPerLeaf = num;
	}

	private int getMinimumRecordsPerLeaf() {
	...
	*/
	private EntrSplit findSplitValue(Table t, int attCol, int outCol, int[] examples) {
        ...
        */
		...
		double lastTest = t.getDouble(sortedExamples[0], attCol);
	...
	*/
	public void doit() throws Exception {
        ...
        //Integer minRecs = (Integer)pullInput(1);
        SOIntSolution sol = (SOIntSolution)pullInput(1);
        int minRecs = (int)sol.getDoubleParameter(0);
        this.setMinimumRecordsPerLeaf(minRecs);
		...
		if(best != null) {
            ...
            if(!table.isColumnScalar(col)) {
				...
				try {
					...
					String val = 
						mostCommonOutputValue(table, outputs[0], examples);
					DecisionTreeNode nde = new CategoricalDecisionTreeNode(val);
					root.addBranch(val, nde);
