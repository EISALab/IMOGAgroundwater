...
import ncsa.d2k.modules.core.datatype.table.sparse.columns.SparseDoubleColumn;
...
public class VIntDoubleHashMap
  ...
  /**
   * Creates a new <code>VIntDoubleHashMap</code> instance with the default
   * capacity and load factor.
   */
  public VIntDoubleHashMap() {
    super();
  }

  /**
   * Creates a new <code>VIntDoubleHashMap</code> instance with a prime
   * capacity equal to or greater than <tt>initialCapacity</tt> and
   * with the default load factor.
   *
   * @param initialCapacity an <code>int</code> value
   */
  public VIntDoubleHashMap(int initialCapacity) {
   ...
   */
  public VIntDoubleHashMap(int initialCapacity, float loadFactor) {
   ...
   * @return the value of <tt>key</tt> or minimum value of Double if no such
   * mapping exists.
   */
  public double get(int key) {
    int index = index(key);
    return index < 0 ? SparseDefaultValues.getDefaultDouble() : _values[index];
  }

  public VIntDoubleHashMap copy() {
    VIntDoubleHashMap newMap;
    try {
      ...
      newMap = (VIntDoubleHashMap) ois.readObject();
      ois.close();
      return newMap;
    }
    catch (Exception e) {

      newMap = new VIntDoubleHashMap();
      newMap._free = _free;
      ...
      newMap._values = new double[_values.length];
  ...
  /**
   * Returns a new VIntDoubleHashMap with reordered mapping as defined by
   * <code>newOrder</code>
   *
   ...
   *                  returned value.
   * @return          a VIntDoubleHashMap with the same values as this one,
   *                  reordered.
   */
   ...
   * @return         a sorted double array with the values mapped to keys <code>begim
  ...
  public double[] getValuesInRange(int begin, int end) {
    ...
    double[] values = new double[keysInRange.length];
  ...
  public VIntIntHashMap getSortedOrder(int begin, int end) {
    ...
    //sorting the values
    double[] values = getValuesInRange(begin, end);
   ...
   *               the subset
   * @return       a VIntDoubleHashMap with values and keys from this map, s.t.
  ...
  public double remove(int key) {
    double prev = SparseDefaultValues.getDefaultDouble();
    int index = index(key);
    if (index >= 0) {
      prev = _values[index];
   ...
   *                    that is mapped to y.
   */
  private VIntIntHashMap getSortedOrder(int[] validKeys, double[] values) {
    ...
    double currVal; //current value for which its place is searched
   ...
   */
  private int getNewKey(double currVal, double[] values, int key,
