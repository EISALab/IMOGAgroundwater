...
public class SQLRainForestOPT extends ReentrantComputeModule {
  ...
  */
  ...
  public void doit() throws Exception {
    ...
    if(meta.isColumnScalar(outputFeatures[0])) {
      ...
      setMinimumRatioPerLeaf(pp.getValue(SQLRainForestParamSpaceGenerator.
                                         MIN_RATIO));
      minimumRecordsPerLeaf = totalRow * minimumRatioPerLeaf;
      setModeThreshold(pp.getValue(SQLRainForestParamSpaceGenerator.
                                   MODE_THRESHOLD));
      setBinNumber(pp.getValue(SQLRainForestParamSpaceGenerator.BIN_NUMBER));
      setDominateRatio(pp.getValue(SQLRainForestParamSpaceGenerator.
                                   DOMINATE_RATIO));
  ...
  private String[] getClassValues(String classColName) {
    ...
    try {
      ...
      String classQry = new String("select distinct " + classColName + " from " + dbTable +
                        " where " + classColName + " is not null");
  ...
  private NodeInfo extractDataFromDB(String dbTable, String classColName, ArrayList path,
    ...
    try {
      ...
      dataQry = dataQry + " from " + dbTable + " where " + classColName + " is not null";
      if (path.size() > 0) {
