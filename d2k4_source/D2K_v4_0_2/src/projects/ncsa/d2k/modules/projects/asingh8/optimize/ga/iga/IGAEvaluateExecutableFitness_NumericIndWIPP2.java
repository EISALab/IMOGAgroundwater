...
public class IGAEvaluateExecutableFitness_NumericIndWIPP2 extends ncsa.d2k.core.modules.ComputeModule{
        int numExecFitness;
        
        MutableTableImpl fitnessExecs = new MutableTableImpl();// new TableImpl();
        MutableTableImpl interpFileNames ;
        ...
        String interpInputFileName;
        String[][] execOutputFileNames ;
        String [] interpOutputFileNames;
	...
	public String getInputInfo(int index) {
		switch (index) {
			...
			case 1: return "A TableImpl data structure that contains fitness ids for fitnesses evaluated using the executables and the entire paths names for executables with runtime arguments.";
                        case 2: return "A TableImpl data structure that contains file names for interpolation";
	...
	*/
	...
	public String getModuleName() {
		return "IGA: EvaluateExecutableFitness_NumericIndWIPP";
	}
		...
		switch(index) {
			case 0:
				...
				return "Fitness executable table";
                        case 2:
                                return "Interpolation File Table";
			default: return "NO SUCH INPUT!";
		}
	}
	...
	public void wipeFields(){
            numExecFitness = 0;
            fitnessExecs = null;
            interpFileNames = null;
            execFitnessIds = null;
            numOutputFiles = null;
            numInterpFiles = 0;
            execPathNames = null;
            execInputFileNames = null;
            interpOutputFileNames = null;
            execOutputFileNames = null;
            interpInputFileName = null;
	...
	*/
       public void doit () throws Exception {
          ...
          //  System.out.println("entering doit function of EvaluateExecutableFitness");
            ...
            if(interpFileNames == null) {
                interpFileNames = (MutableTableImpl) this.pullInput(2);
           ...
           // fitnessExecs = (MutableTableImpl) this.pullInput(1);
            ...
            interpInputFileName = this.getInterpInputFileName();

            execOutputFileNames = new String[getNumFitnessExecs()][10];
            execOutputFileNames = this.getOutputFileNames();
            
            interpOutputFileNames = new String[getNumInterpFiles()];
            interpOutputFileNames = this.getInterpFileNames();
                                
            numOutputFiles= new int[getNumFitnessExecs()];
            numOutputFiles= this.getNumOutputFiles();
            
            numInterpFiles = this.getNumInterpFiles();
            ...
            this.writeGenesToFile(pop,execInputFileNames, interpInputFileName);
	...
	public void evaluateIndividual (Population popul) {//(NsgaPopulation popul) { //
        ...
        */
         ...
         * @throws IOException
         */        
        public void writeGenesToFile (Population popul, String[] execInputFileNames, String interpInputFileName) throws IOException {
        //(NsgaPopulation popul, String[] execInputFileNames) throws IOException {

          //  String genes = new String() ;
            interpFileNamesTable = new MutableTableImpl(this.getNumInterpFiles());
            
            String [][] interpFiles = new String [popul.size()][this.getNumInterpFiles()];
             ...
             for (int i = 0; i < popul.size(); i++){
                 ...
                 int counter = 0;
                    ...
                    for (int k=0; k < this.getNumInterpFiles(); k++){
                        String dest = new String (interpOutputFileNames[k]+"_"+"Ind"+ i  );
            ...
            for(int i = 0; i < getNumInterpFiles() ; i++){
                  String[] colDat = new String [popul.size()];
                  for (int j=0; j< popul.size(); j++){
            ...
            try{
                if(interpInputFileName != null){
                       FileWriter stringFileWriter = new FileWriter(interpInputFileName);
                       BufferedWriter bw = new BufferedWriter(stringFileWriter);
                       PrintWriter pw = new PrintWriter(bw);
                       
                       pw.println(getNumInterpFiles());
...
//(NsgaPopulation popul, String outFileName ) throws IOException {
         ...
         * an executable. These files would contain the fitness values after evaluation.
         */
        public String[] getInterpFileNames() {
           int numFiles = 0;
           numFiles = this.getNumInterpFiles();
           String[] InterpOutputFileName = new String[getNumInterpFiles()];
           
              for ( int j = 0; j < numFiles; j++ ){
                  InterpOutputFileName[j] = interpFileNames.getString(0,j+2);
              }
           
           return InterpOutputFileName;
        }
        ...
        public int getNumInterpFiles() {
          int size;
          size = interpFileNames.getInt(0,0) ;
          return size;
        }
        ...
        public String getInterpInputFileName() {
           String interpInputFileName;
           
               interpInputFileName = interpFileNames.getString(0,1);
           
           return interpInputFileName;
        }
