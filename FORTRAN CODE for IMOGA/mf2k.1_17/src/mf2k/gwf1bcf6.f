...
C9------RETURN.
     ...
     1 SC2,TRPY,IN,ISS,NCOL,NROW,NLAY,IOUT,WETDRY,IWDFLG,CVWD, cond_val)
C
...
C Added by Abhishek Singh to allow conductivities to be read in
	REAL cond_val(NROW,NCOL,NLAY)
...
C     Added by Abhishek Singh to over write CC values
      DO JJ = 1,NCOL
	 DO II = 1,NROW
      	CC(JJ,II,K) = cond_val(NROW-II+1,JJ,K)
	 END DO
	END DO
...
C     Added by Abhishek Singh to over write HY values
      DO JJ = 1,NCOL
	 DO II = 1,NROW

        HY(JJ,II,K) = cond_val(NROW-II+1,JJ,K)
