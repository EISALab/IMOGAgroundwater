...
C-------ASSIGN VERSION NUMBER AND DATE
	SUBROUTINE modflow_sub(NAMEFILE, HfrmModf, NCOL2, NROW2,
     &Nlay2, cond_val)
...
C     Added by Abhishek Singh and Eva Sinha to change program into subroutine
	...
	INTEGER NCOL2, NROW2, NPER2
	REAL, INTENT(INOUT) :: HfrmModf(NROW2, NCOL2)
	REAL :: cond_val(Nrow2,Ncol2,Nlay2)
	REAL TEMP1
...
C2------OPEN FILE OF FILE NAMES.
   10 CONTINUE
      IF (MYID.EQ.MPROC) THEN
        IF (IBATCH.GT.0) THEN
C	COMMENTED OUT READ FNAME (ABHISHEK)
...
c            READ (*,'(A)') FNAME
c            CALL URWORD(FNAME,ICOL,ISTART,ISTOP,0,N,R,0,0)
c            FNAME=FNAME(ISTART:ISTOP)
          ENDIF
c	MAKE FNAME = INPUT FILE NAME (ABHISHEK)
	    FNAME = NAMEFILE
          IF (FNAME.EQ.' ') GOTO 15
...
C     BROADCAST FNAME AND OPEN FILE FOR WARNINGS AND ERROR MESSAGES
      ...
      IF (MYID.EQ.MPROC) WRITE(*,*)' Using NAME file: ',FNAME(1:25)
  490 FORMAT(A,A)
C
...
C---------BASIC PACKAGE
     ...
     5                   IAUXSV,RESETDD,RESETDDNEXT)
        IF(IUNIT(1).GT.0)
C Added cond_val in the input parameters for BCF5RP by Abhishek Singh
...
C7C6----IF ITERATION FAILED TO CONVERGE THEN STOP.
              IF (ICNVG.EQ.0) then 
C	ADDED BY ABHISHEK SINGH TO STOP END OF MODFLOW
...
C      CALL USTOP(' ')
C
C     added by Abhishek Singh to close files
  	CLOSE(INBAS)
	DO I = 1,40
		IF(IUNIT(I) > 0) then
		   CLOSE(IUNIT(I))
C		   WRITE(*,*) 'CLOSING', IUNIT(I)
	    END IF
	END DO
