README.TXT
                  ...
                  MODFLOW-2000 - Version: 1.17.01 09/22/2006
...
It is recommended that no user files are kept in the mf2k.1_17 directory
structure.  If you do plan to put files in the mf2k.1_17 directory
structure, do so only by creating subdirectories.

Included in directory mf2k.1_17\doc are various documentation files.  Some
...
code is provided in the mf2k.1_17\src directory so that the programs can
...
one are in the mf2k.1_17\src\mf2k\ directory.  In addition to the files in 
the mf2k.1_17\src\mf2k\ directory,the file para-non.f in the  
mf2k.1_17\src\mf2k\serial\ directory must be compiled.
...
directory, the directory containing the executables (mf2k.1_17\bin)
...
As an alternative, all of the files in the mf2k.1_17\bin directory can
...
AUTOEXEC.BAT file:
  ...
  PATH=%PATH%;C:\WRDAPP\mf2k.1_17\bin
...
";C:\WRDAPP\mf2k.1_17\bin" to its definition in the Value field, and click
...
window, add ";C:\WRDAPP\mf2k.1_17\bin" to the end of the Variable Value
...
Fortran 95 extended memory compiler version 5.70d.
...
After the files in the mf2k.1_17\bin directory are installed in a
...
at as examples of how to use the program.  The directory MF2K.1_17\data
contains the input data and expected results for each test.

The directory MF2K.1_17\test-win can be used to conveniently run the
tests without destroying the original results in the MF2K.1_17\data
directory.  MF2K.1_17\test-win contains batch (BAT) files to run the
...
MF2K.1_17\test-win or by double-clicking on the corresponding BAT file
in Windows Explorer.  The output files that are created in
MF2K.1_17\test-win can then be compared to those in MF2K.1_17\data.  
