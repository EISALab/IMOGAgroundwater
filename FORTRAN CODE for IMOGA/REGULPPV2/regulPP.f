      subroutine regulPP(outp, numpp, regul)
C%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
C                                                                      %
...
c-----------------------------------------------------------------------
      include  'gamvPP.inc'
	integer num, len, i, j, gen, numpp
	real diff, dist
	real inp(50)
	real outp(50)
	real regul

	open (2345, file = 'genesV.txt', status = 'OLD')
	open (3456, file = 'fitnessV.txt', status = 'UNKNOWN')
	read (2345, *) gen
	read (2345, *) num
	read (2345, *) len
...
c
 ...
 1234 format('gen',i4)
	...
	do i = 1, num
	    ...
	    read(2345, *)
	    read(2345, *)
		call writeout2(inp, outp, regul, numd, idnum)
	    write(3456, *) regul
	end do
c	call writeout
c
...
c
 ...
 9998 format(/' GAMV Version: ',f5.3, ' Finished'/)
      ...
      subroutine readparm2(numd, idnum)
c-----------------------------------------------------------------------
c
...
c-----------------------------------------------------------------------
      ...
      real      var(MV),cut(MXVARG),vrmin(MAXVAR),vrmax(MAXVAR)
...
c            stop
c      endif
      open(lin,file='gamvPP.par',status='OLD')
c
...
c
      ...
      if(nvar.gt.MAXVAR)  stop 'nvar is too big: check gamv1.inc'
      backspace lin
      ...
      if(nlag.gt.MAXLAG)  stop 'nlag is too big: check gamv1.inc'
      ...
      if(ndir.gt.MAXDIR)  stop 'ndir is too big: check gamv1.inc'
     ...
     +                         dip(i),dtol(i),bandwd(i)
      ...
      if(nvarg.gt.MXVARG) stop 'nvarg is too big: check gamv1.inc'
...
c PROGRAM NOTES:
c
c   1. The file "gamv1.inc" contains the dimensioning parameters.
...
c-----------------------------------------------------------------------
      ...
      real      uvxazm(MAXDIR),uvyazm(MAXDIR),uvzdec(MAXDIR),
     +          uvhdec(MAXDIR),csatol(MAXDIR),csdtol(MAXDIR)
      logical   omni
c
...
c
      ...
      if(nsiz.gt.MXDLV) then
            write(*,*) 'ERROR: available storage in gamv = ',MXDLV
            write(*,*) '       requested storage         = ',nsiz
            write(*,*) '       directions                = ',ndir
            write(*,*) '       variograms                = ',nvarg
            write(*,*) '       MAXLG                     = ',MAXLG 
            stop
      endif
      do i=1,nsiz
...
c
 ...
 7    continue
      ...
      subroutine writeout2(inp, outp, regul,numd, idnum)
c-----------------------------------------------------------------------
c
...
c-----------------------------------------------------------------------
      ...
      character title*132
      data      lout/1/
	real      moddist, cova, val1, val2
	real      inp(50)
	real	  outp(50)
	real      pi, error, count, regul, covamax
	integer   ij, i, j
...
c            do il=1,nlag+2
	  ...
	  do j = 1,nd
	    ...
	    val1 = vr(i,1)
	    val2 = vr(j,1)
c	    if(idnum(i).lt.0) val1 = inp(14+i-numd)
c	    if(idnum(j).lt.0) val2 = inp(14+j-numd)
	    val1 = outp(i)
	    val2 = outp(j)
	    diff = val1*val2
...
c                  i = (id-1)*nvarg*MAXLG+(iv-1)*MAXLG+il
c                  nump = int(np(i))
c                  if(it.eq.4) then
...
c  compute anisotropic distance
	...
	moddist = ((cos((azm(id)-inp(12+ij*5))*pi/180)/inp(13+ij*5))**2 
     + +(sin((azm(id)-inp(12+ij*5))*pi/180)/inp(14+ij*5))**2 )**0.5
	moddist = dist*moddist
c Spherical Variogram Model?
c
          if(inp(10+ij*5).eq.1) then
c                  hr = h/aa(ist)
            if(moddist.lt.1.) then
		  cova=cova+inp(11+ij*5)*(moddist*(1.5-.5*moddist**2))
            else
		  cova = cova+inp(11+ij*5)
		  end if 

c
c Exponential Variogram Model?
c
          else if(inp(10+ij*5).eq.2) then
                  cova = cova + inp(11+ij*5)*(1-exp(-3.0*moddist))
c
c Gaussian Variogram Model?
c
          else if(inp(10+ij*5).eq.3) then
                  cova = cova + inp(11+ij*5)*(1-exp(-(3.0*moddist)
     +                                      *(3.0*moddist)) )
	    end if

	covamax = covamax+inp(11+ij*5)

	end do
	       regul = regul + diff/(cova)
	       count = count + 1

	        write(lout,*) count,dist,diff,covamax-cova	       


c                   endif
 102  format(1x,i3,1x,f12.3,1x,f12.5,1x,f12.5,1x,i8,4(1x,f14.5))
        end do
	  end do
