...
C        SPECIFICATIONS:
C     ------------------------------------------------------------------
      CHARACTER*12 OBSNAM(ND), CTMP1, CTMP2, NAM(ND) ! swm: added NAM
c swm      CHARACTER(LEN=12), ALLOCATABLE :: NAM(:)
...
c swm added lines below
          LENG= NONB_LEN(OUTNAM,200)
c swm          FN = TRIM(OUTNAM)//'._wt'
          FN = OUTNAM(1:LENG)//'._wt'
          OPEN(IWTF,FILE=FN)
