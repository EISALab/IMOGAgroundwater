...
c      PARAMETER (LENGX=10000000, LENIG=10000000, LENGZ=10000000,
c     &           LENX=20000000, LENIX=15000000, LENZ=10000000,
...
C4------ALLOCATE SPACE IN RX AND IR ARRAYS.
     ...
     &                  IUNIT(54))
cswm        IF(IUNIT(54).GT.0)
c     1      CALL GWF1SUB1ALP(NROW,NCOL,NLAY,ITERP,ISUBCB,ISUBOC,AC1,AC2,
c     2                  ITMIN,NNDB,NDB,NPZ,NN,NND1,ND1,ND2,IDSAVE,
c     3                  IDREST,ISSFLG,NPER,NSTP,NSTPT,IUNIT(54),IOUT,
c swm     4                  IUNIT(9),LCV,ISEN)
        IF(IUNIT(20).GT.0)
...
cswm          IF (ITERPK.EQ.1) ALLOCATE (MNWSITE(MXWEL2))
        ENDIF
C
...
c swm        IF(IUNIT(54).GT.0)
c     1      CALL GWF1SUB1RPP(GX(LCDELR),GX(LCDELC),GZ(LCHNEW),
c     2                  GX(LCBUFF),NCOL,NROW,NLAY,NODES,NPER,NSTP,
c     3                  ISUBOC,NND1,ND1,ND2,NDB,NNDB,NPZ,NN,IDSAVE,
c swm     4                  IDREST,NSTPT,IUNIT(54),IOUT)
        IF(IUNIT(20).GT.0)
...
c swm          IF(IUNIT(54).GT.0)
c     1        CALL GWF1SUB1ST(GZ(LCHNEW),NNDB,NDB,ISSFLG,NROW,NCOL,
c swm     1                        NODES,NPER,KPER,NN)
...
c swm              IF(IUNIT(54).GT.0)
c     1            CALL GWF1SUB1FM(GX(LCRHS),GX(LCHCOF),GZ(LCHNEW),
c     2                            GX(LCHOLD),IG(LCIBOU),X(LCV),
c     3                            GX(LCDELR),GX(LCDELC),NCOL,NROW,
c     4                            NODES,DELT,AC1,AC2,HCLOSE,KKITER,
c     5                            ITMIN,NN,NND1,ND1,ND2,NDB,NNDB,NPZ,
c swm     6                            ISSFLG(KKPER),IUNIT(9))
CLAK
              IF (IUNIT(22).GT.0)
...
C--INTERBED STORAGE
     ...
     6                          IBSDIM)
c swm            IF(IUNIT(54).GT.0)
c     1          CALL GWF1SUB1BD(IG(LCIBOU),GZ(LCHNEW),GX(LCHOLD),
c     2                          GX(LCBUFF),GX(LCDELR),GX(LCDELC),VBVL,
c     3                          VBNM,NN,NND1,ND1,ND2,NDB,NNDB,NPZ,NCOL,
c     4                          NROW,NLAY,NODES,DELT,MSUM,NIUNIT,KKSTP,
c swm     5                          KKPER,ISUBCB,ICBCFL,ISSFLG(KKPER),IOUT)
C--LAKES
            IF (IUNIT(22).GT.0)
...
c swm            IF(IUNIT(54).GT.0)
c     1          CALL GWF1SUB1OT(NCOL,NROW,NLAY,PERTIM,TOTIM,KKSTP,
c     2                          KKPER,NSTP(KKPER),GX(LCBUFF),NODES,NN,
c     3                          ND1,ND2,NND1,NNDB,NDB,ISSFLG(KKPER),
c swm     4                          IUNIT(54),IOUT)
...
c swm      IF(IUNIT(54).GT.0) CALL GWF1SUB1SV(ND2,IDSAVE)
