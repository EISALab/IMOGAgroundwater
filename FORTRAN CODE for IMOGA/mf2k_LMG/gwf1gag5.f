...
C TEMPORARY ARRAYS
cswm      ALLOCATABLE CONCNAME(:),DCTSNAME(:),DCCMNAME(:),DUMMY(:,:)
      DIMENSION IGGLST(3,NUMGAGE),VOL(NLAKES)
      DIMENSION STAGES(NLAKES),CLAKE(NLAKES,NSOL),ISTRM(5,NSTRM)
C ALLOCATE TEMPORARY ARRAYS
cswm      ALLOCATE(CONCNAME(NSOL),DCTSNAME(NSOL),DCCMNAME(NSOL),
cswm     *DUMMY(NLAKES,NSOL))
      DIMENSION CONCNAME(NSOL),DCTSNAME(NSOL),DCCMNAME(NSOL),
     &        DUMMY(NLAKES,NSOL)
C
C     ******************************************************************
C
c swm:  added loops below
      DO I=1,NSOL
        DO J=1, NLAKES
          DUMMY(J,I)=0.0
        ENDDO
...
C
C  RELEASE MEMORY
c swm      DEALLOCATE(CONCNAME,DCTSNAME,DCCMNAME,DUMMY)
      RETURN
      END
C
C
...
c swm      ALLOCATABLE DELCTS(:,:),DELCCUM(:,:)
c swm      ALLOCATE(DELCTS(NLAKES,NSOL),DELCCUM(NLAKES,NSOL))
      DIMENSION DELCTS(NLAKES,NSOL),DELCCUM(NLAKES,NSOL)
C
C     ******************************************************************
C
C  LOOP OVER GAGING STATIONS
...
C
C  RELEASE MEMORY
c swm      DEALLOCATE(DELCTS,DELCCUM)
      RETURN
      END
C
C
