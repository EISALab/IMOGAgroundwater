...
c-----------------------------------------------------------------------
      include  'gamv1.inc'
	integer num, len, i, j, gen, dummy
	real inp(50)
	real outp, error

	open (2345, file = 'genesV.txt', status = 'OLD')
	open (3456, file = 'fitnessV2.txt', status = 'UNKNOWN')
	read (2345, *) gen
	read (2345, *) num
	read (2345, *) len
...
c
 ...
 1234 format('gen',i4)
	...
	do i = 1, num
	    ...
	    read(2345, *)
	    read(2345, *)
		call writeout(inp, error)
c	    write(3456, *) log10(error)
		write (3456, *) dummy
...
c            stop
c      endif
      open(lin,file='gamvV.par',status='OLD')
c
...
c
     ...
     +                                      *(3.0*moddist)) )
	        ...
	        write(lout,102) il,dis(i),gam(i),cova, nump,
     +                                  hm(i),tm(i)
	       if(dis(i).gt.0) then
	       error = error + ((gam(i) - cova)**2)*nump
	       count = count + nump
	       end if
