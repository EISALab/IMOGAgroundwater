...
C
C
      SUBROUTINE BTN4RP(IN,IOUT,IUCN,IOBS,IMAS,ICNF,ICBM,NCOL,NROW,NLAY,
     & NCOMP,LAYCON,DELR,DELC,HTOP,DZ,PRSITY,ICBUND,COLD,CNEW,CADV,
     & CINACT,THKMIN,XBC,YBC,ZBC,RETA,RFMIN,BUFF,MXPRS,NPRS,TIMPRS,
...
C     Commented by Abhishek Singh and Eva Sinha to suppress screen output
C      WRITE(*,70) NTRANS,DTRANS,TIME2
...
C
C
C	Added ConcfrmMT3d by Abhishek Singh and Eva Sinha to directly read in the 
C	concentration of two species
      SUBROUTINE BTN4OT(NCOL,NROW,NLAY,KPER,KSTP,NTRANS,NCOMP,ICOMP,
     & IOUT,IOBS,IUCN,IMAS,ICBM,MXOBS,NOBS,NPROBS,LOCOBS,ICBUND,TIME2,
     & CNEW,MIXELM,NCOUNT,NPINS,NRC,NPCHEK,ISOTHM,RETA,TMASIN,TMASOT,
     & ERROR,ERROR2,TRNOP,TUNIT,MUNIT,PRTOUT,TMASIO,RMASIO,TMASS,
...
C
     ...
     &          NPRMAS,ICBUND,NCOMP,ICOMP, TOTYEARS
	...
	REAL      CONCFRMMT3D(NROW,NCOL,TOTYEARS,NCOMP)
...
C--IF REQUESTED
C	Added by Abhishek Singh and Eva Sinha to save concentration in array
	DO I = 1,NROW
	  DO J = 1,NCOL
		CONCFRMMT3D(I,J,CEILING(time2),ICOMP) = CNEW(J,I,1,ICOMP)
	  END DO
	END DO
C
