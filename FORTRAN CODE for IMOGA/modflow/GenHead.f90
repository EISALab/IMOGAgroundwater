...
! shift these to a param file, so that they can be changed when needed.
      ...
      integer :: PmpZon(OriNrow1,OriNcol1), Buff(OriNrow1,OriNcol1)
      ...
      integer :: ir,ic,iy,iy_start,iy_end,K,I,J,Year, temp_int, iter, i2
      ...
      real :: CRdx(OriNrow1,OriNcol1,MxYr1), CTnt(OriNrow1,OriNcol1,MxYr1)
      ...
      real, intent(inout) :: Total_cost

	  
	  real, allocatable :: HEADfrmMODF(:,:,:), horiz_cond(:,:,:)
...
! some changed will have to be made to the input file as well.
	...
	!do i2 = 1,iter
	HEADfrmMODF = 0.00d0
	call modflow_sub(namefile, HEADfrmMODF, OriNcol1, OriNrow1, Nstress,OriNlay1, horiz_cond)
	CONCfrmMT3D = 0.00d0
  ...
  ! Opening the response file for the input information
   ...
   OPEN(UNIT = 101, FILE =temp_flname, STATUS = 'UNKNOWN')
...
! ----------------
      open(iout,file=out_fil,status='unknown')
...
! ------------------------------------------------------------
      write(iout,6)
      do K = 1,Nstress
        iy_start = (K-1)*PeriodLength+1
        iy_end = K*PeriodLength
        write(iout,7) K
        do i = 1,Nwel(K)
	  ir = LocWel(2,K,I)
	  ic = LocWel(3,K,I)
...
!         Delc(OriNrow1) = Grid Spacing Along Column
      ...
      implicit None
      character filename*40,fmtin*20,idbg, temp_flname*80
 ...
 !       read(in,end=888) ((c(i,j,iy),j=1,OriNcol1),i=1,OriNrow1)
       do i = 1,OriNrow1
	    do j = 1,OriNcol1
		 ...
		 if(c(i,j,iy)<0.0d0) c(i,j,iy) = 0.0d0
	    end do
	   end do
  ...
  !      write(*,'(/1x,''The max. conc for year # '',i5,f10.3)')iy,cmax
		 ...
		 if(cmax .le. cl .and. ny .le. 0) then
		   ny=iy
		   exit
		 end if
       ...
       do Kper = 1,Nstress
		 do I = 1,OriNrow1
		  do J = 1,OriNcol1
		   ...
		   if(Head(I,j,Kper) .lt.0 .and. Head(I,J,Kper) .ne. -999)then
...
!       Calculate Starting and Ending Years of This Period
          StartYear = (K-1)*PeriodLength+1
          EndYear = K*PeriodLength
...
!      Calc Cost for EW-2
        if (PeriodEw2 .gt. 0 .and. PeriodEw2 .le. Nstress) then
          Year = (PeriodEw2-1)*PeriodLength+1
...
!      Calculate Number of Stress Period When Cleanup
        RealNum = Real(Ny) / Real(PeriodLength)
...
!         Calculate Current Stress Period
          Period = int((Year-1)/PeriodLength) +1
...
!***********************************************************************
       subroutine PenaltyFunc (Ny, TNTarea, Rate, PenaltyF)
!       Calculate the Penalty Function for Leaving a TNT Plume Given:
!         Ny = Number of Years when Done with RDX
!         TNTarea = Area of TNT plume remaining
!         Rate = Discount Rate

        implicit none
        integer :: Ny
        real :: TNTarea, Rate, Cost, PenaltyF
        real :: Pv

!       Check for Area = 0, No Penalty
        if (TNTarea .le. 0.0) then
          PenaltyF = 0.0
          return
        endif

!       Calculate Cost of Leaving TNT Plume
        Cost = 96.0 + 0.0028 * TNTarea

!       Convert to Present Value
        PenaltyF =  Cost / ((1+Rate)**(Year-1))

        return
      end subroutine PenaltyFunc
