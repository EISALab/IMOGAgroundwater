...
c
      open (2345, file = 'genes_interpV.txt', status = 'OLD')
c	open (3456, file = 'fitnessV.txt', status = 'UNKNOWN')
	read (2345, *) num
	read (2345, *) len
	read (2345, *) no
	...
	do i = 1, num
	            outptfl = '                                         '
	            read(2345, '(a40)') outptfl
		do j = 1, len
				read(2345, *) inp(j)
		end do
	    read(2345, *)
	    read(2345, *)
	    call readparm(inp, outptfl)
	    call kt3d
...
c      stop
	...
	close(3456)
      ...
      subroutine readparm(inp, outptfl)
c-----------------------------------------------------------------------
c
...
C            stop
C      endif
      open(lin,file='kt3dV.par',status='OLD')
c
...
c
	...
	call chknam(outptfl, 40)
      write(*,*) ' output file = ',outfl
	write(*,*) 'interp output file = ', outptfl
	...
	ndmin = min(inp(1), inp(2))
	ndmin = 0
	ndmax = max(inp(2), inp(1))
...
C           read(lin,*,err=98) aa(i),aa1,aa2
	      aa(i) = max(inp(13+(i-1)*5), inp(14+(i-1)*5))
	      aa1 = min(inp(13+(i-1)*5), inp(14+(i-1)*5))
	      aa2 = 0
...
c
	...
	open(3456, file = outptfl, status = 'UNKNOWN')
...
c
 1          continue
            if(iktype.eq.0) then
                  if(koption.eq.0) then
                        write(lout,'(f11.5,1x,f11.5)') est,estv
                  else
                        err = UNEST
                        if(true.ne.UNEST.and.est.ne.UNEST)err=est-true
                        write(lout,'(7(f14.5,1x))') xloc,yloc,zloc,true,
     +                                        est,estv,err
...
c      open(lout,file=outfl,status='OLD')
c	open(3456, file = outptfl, status = 'unknown')
	...
	do i = 1, nz
	 do j = 1, ny
	  do k = 1, nx
           read(lout,'(f11.5,1x,f11.5)') est,estv
	     if(est.ne.-999) est = exp(est+estv/2)
	     write(3456, 9001 ) est
9001	format (f14.10, \)
	  end do
	     write(3456, *) ' '
